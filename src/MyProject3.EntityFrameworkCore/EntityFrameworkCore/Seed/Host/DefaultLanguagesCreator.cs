...
namespace MyProject3.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly MyProject3DbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = MyProject3Consts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(MyProject3DbContext context)
        {
            _context = context;
        }
